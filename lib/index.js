...
const koaMiddleware = async (ctx, next, {
  ...
  if (colonIdx > 0) {
    ...
    const rpcVerb = url.slice(colonIdx + 1, queryStartIdx > 0 ? queryStartIdx : undefined);
...
const expressMiddleware = (req, next, {
  allowedRpcVerbs,
  disableHeader
}) => {
  const {
...
const parseRpcVerbFromUrl = (options = {}) => {
  ...
  return async (arg1, arg2, arg3) => {
    const engine = !!arg2 && typeof arg2 === 'function' ? 'koa' : 'express';
    if (engine === 'koa') {
